...
def build_traj_image(x_list,
                     ...
                     traj_list=None):
    if image_mode == "binary":
        return image_encode(x_list, y_list, x_num_classes, y_num_classes)
    if image_mode == "motion6":
        return image_encode_motion_canvas(x_list, y_list, x_num_classes, y_num_classes, traj_list)
    if image_mode == "motion6_pyr2":
        return image_encode_motion_canvas_pyramid(x_list, y_list, x_num_classes, y_num_classes, traj_list, scale_factors=(1, 2))
    if image_mode == "multigrid3":
        return image_encode_multigrid_canvas(x_list, y_list, x_num_classes, y_num_classes)
    raise ValueError("Unsupported image_mode: {}".format(image_mode))
