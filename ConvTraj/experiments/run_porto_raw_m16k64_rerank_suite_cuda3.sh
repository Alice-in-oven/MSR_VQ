...
TRAIN_FLAG="porto_raw_m16k64_decoded_rerankL1000_cuda3"
EXP_ROOT="$ROOT/exp/$TRAIN_FLAG"
DEVICE="cuda:3"
...
echo "[STEP 1/2] Train + strict decoded rerank L=1000 on $DEVICE"
python -u train.py \
  ...
  --decoded_ste_metric_start_epoch 5 \
  --decoded_ste_metric_warmup_epochs 35 \
  --decoded_ste_metric_max_weight 0.12 \
  ...
  --porto_opq_teacher_end_epoch 120 \
  --porto_opq_teacher_z_weight 0.30 \
  --porto_opq_teacher_partition_weight 0.30 \
  --late_finetune_start_epoch 360 \
  --late_finetune_main_lr_scale 0.2 \
  --late_finetune_pre_quant_lr_scale 0.2

echo "[STEP 2/2] Hybrid eval: continuous coarse shortlist + decoded rerank L=1000"
python -u experiments/eval_porto_continuous_coarse_decoded_rerank.py \
  --root_write_path "$EXP_ROOT" \
  --train_flag "$TRAIN_FLAG" \
  --epoch 400 \
  --dist_type dtw \
  --gt_path "$GT_PATH" \
  --rerank_L 1000 \
  --output_json "$EXP_ROOT/feature_dir/metrics_hybrid_continuous_coarse_decoded_L1000_${TRAIN_FLAG}_e400.json"
