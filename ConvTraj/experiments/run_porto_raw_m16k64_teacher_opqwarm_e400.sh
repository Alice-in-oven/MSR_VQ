...
EXP_ROOT="$ROOT/exp/porto_raw_m16k64_teacher_opqwarm_e400"
TRAIN_FLAG="porto_raw_m16k64_teacher_opqwarm_e400_cuda1"
DEVICE="cuda:1"
...
PORTO_OPQ_TRAIN_RECON="$ROOT/exp/porto_raw_backbone400_faiss_m16_nbits6_opq/porto_raw_backbone_cont_opq_m16_nbits6_train_faiss_recon.pkl"
...
python -u train.py \
  ...
  --pdt_loss_weight 0.1 \
  --pdt_loss_start_epoch 10 \
  --decoded_ste_metric_enabled \
  --decoded_ste_metric_start_epoch 30 \
  --decoded_ste_metric_warmup_epochs 30 \
  --decoded_ste_metric_max_weight 0.10 \
  --porto_opq_warmup_train_recon_path "$PORTO_OPQ_TRAIN_RECON" \
  --porto_opq_warmup_start_epoch 0 \
  --porto_opq_warmup_end_epoch 40 \
  --porto_opq_warmup_max_weight 0.10 \
